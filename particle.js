...
import {
  ...
  lowerSplashFragment,
  higherSplashVertex,
  higherSplashFragment,
  dropletVertex,
  dropletFragment,
  dropletRippleVertex,
  dropletRippleFragment,
  floatingSplashVertex,
  floatingSplashFragment,
  swimmingSplashVertex,
  swimmingSplashFragment,
  swimmingRippleVertex,
  swimmingRippleFragment,
  bubbleVertex,
  bubbleFragment,
} from './shaders.js';

const {useLoaders, useSound} = metaversefile;
const baseUrl = import.meta.url.replace(/(\/)[^\/\\]*$/, '$1');
const sounds = useSound();
const soundFiles = sounds.getSoundFiles();

const bubbleTexture1 = loadTexture('assets/textures/Bubble3.png');
const bubbleTexture2 = loadTexture('assets/textures/Bubble2.png');
...
const noiseMap3 = loadTexture('assets/textures/noise3.png');
const maskTexture = loadTexture('assets/textures/mask.png');
const splashTexture = loadTexture('assets/textures/splash1.png');
const splashTexture2 = loadTexture('assets/textures/splash2.png');
...
class ParticleEffect {
  ...
  tracePlayerInfo() {
    ...
    if (this.player.avatar) {
      this.currentSpeed = this.localVector5
        .set(this.player.avatar.velocity.x, 0, this.player.avatar.velocity.z)
        .length();
      this.playerHeadPos.setFromMatrixPosition(
        this.player.avatar.modelBoneOutputs.Head.matrixWorld
      );
    }
  }
  update(timestamp) {
      ...
      if (this.rippleMesh) {
        ...
        if (this.rippleMesh.material.uniforms.vBroken.value < 1) {
          if (this.rippleMesh.scale.x > 0.15 * (1 + falling * 0.1))
            this.rippleMesh.material.uniforms.vBroken.value =
    ...
    const _handleLowerSplash = () => {
      if (this.lowerSplash) {
        ...
        for (let i = 0; i < particleCount; i++) {
          if (this.contactWater && this.lastContactWater !== this.contactWater) {
            if (this.fallindSpeed > 6) {
              this.lowerSplash.info.velocity[i].x =
                Math.sin(i) * 0.1 + (Math.random() - 0.5) * 0.01;
              this.lowerSplash.info.velocity[i].y = 0.15 * Math.random();
              this.lowerSplash.info.velocity[i].z =
                Math.cos(i) * 0.1 + (Math.random() - 0.5) * 0.01;
        ...
        this.lowerSplash.material.uniforms.cameraBillboardQuaternion.value.copy(
    ...
    const _handleHigherSplash = () => {
      if (this.higherSplash) {
        ...
        this.higherSplash.material.uniforms.cameraBillboardQuaternion.value.copy(
          this.camera.quaternion
        );
      ...
      if (this.droplet && this.dropletRipple) {
        ...
        for (let i = 0; i < particleCount; i++) {
          ...
          if (positionsAttribute.getY(i) >= -100) {
            ...
            if (this.droplet.info.startTime[i] % 2 === 0) this.droplet.info.offset[i] += 1;
            if (this.droplet.info.offset[i] >= 30) {
              this.droplet.info.offset[i] = 0;
            }
            offsetAttribute.setXY(
              i,
              5 / 6 - Math.floor(this.droplet.info.offset[i] / 6) * (1 / 6),
              Math.floor(this.droplet.info.offset[i] % 5) * 0.2
            );
          }
          if (
            positionsAttribute.getY(i) < 0 &&
            !this.droplet.info.alreadyHaveRipple[i] &&
            scalesAttribute.getX(i) > 0.001
          ) {
            scalesAttribute.setX(i, 0.0001);
            ripplePositionsAttribute.setXYZ(
              i,
              positionsAttribute.getX(i),
              0.01,
              positionsAttribute.getZ(i)
    ...
    const _handleFloatingSplash = () => {
      if (this.floatingSplash) {
        ...
        const textureRotationAttribute =
          this.floatingSplash.geometry.getAttribute('textureRotation');
        ...
        for (let i = 0; i < particleCount; i++) {
          ...
          if (brokenAttribute.getX(i) < 1) brokenAttribute.setX(i, brokenAttribute.getX(i) + 0.005);
        }
        const emmitDelay = this.currentSpeed > 0.3 ? 100 : 200;
        if (
          timestamp - this.floatingSplash.info.lastEmmitTime >
            emmitDelay * Math.pow(1.1 - this.currentSpeed, 0.3) &&
          ...
          if (
            ...
            let brokenDegree =
              this.currentSpeed > 0.3 ? 0.23 + 0.2 * Math.random() : 0.4 + 0.3 * Math.random();
    ...
    const _handleSwimmingSplash = () => {
      this.localVector2
        .set(this.playerDir.x, this.playerDir.y, this.playerDir.z)
        .applyQuaternion(this.rotateY);
      if (this.swimmingSplash) {
        ...
        const textureRotationAttribute =
          this.swimmingSplash.geometry.getAttribute('textureRotation');
        const particleCount = this.swimmingSplash.info.particleCount;
        if (this.player.hasAction('swim')) {
          if (this.player.getAction('swim').onSurface && !this.player.hasAction('fly')) {
            if (this.player.getAction('swim').animationType === 'freestyle') {
              if (this.player.avatarCharacterSfx.currentSwimmingHand !== this.lastStroke) {
                let currentEmmit = 0;
                for (let i = 0; i < particleCount; i++) {
                  if (brokenAttribute.getX(i) >= 1) {
                    if (this.player.avatarCharacterSfx.currentSwimmingHand === 'right') {
                      this.swimmingSplash.info.velocity[i].x =
                        (Math.random() - 0.5) * 0.1 +
                        this.playerDir.x * 0.45 * (1 + this.currentSpeed) +
                        this.localVector2.x * 0.1;
                      this.swimmingSplash.info.velocity[i].y = 0.18 + Math.random() * 0.18;
                      this.swimmingSplash.info.velocity[i].z =
                        (Math.random() - 0.5) * 0.1 +
                        this.playerDir.z * 0.45 * (1 + this.currentSpeed) +
                        this.localVector2.z * 0.1;
                    } else {
                      this.swimmingSplash.info.velocity[i].x =
                        (Math.random() - 0.5) * 0.1 +
                        this.playerDir.x * 0.45 * (1 + this.currentSpeed) -
                    ...
                    positionsAttribute.setXYZ(
                      ...
                      this.particlePosition.x +
                        (Math.random() - 0.5) * 0.1 +
                        this.swimmingSplash.info.velocity[i].x -
                        this.playerDir.x * 0.25,
                      this.particlePosition.y,
                      this.particlePosition.z +
                        (Math.random() - 0.5) * 0.1 +
                        this.swimmingSplash.info.velocity[i].z -
                        this.playerDir.z * 0.25
                    );
            ...
            if (this.currentSpeed > 0.3) {
              const splashposition =
                this.player.getAction('swim').animationType === 'breaststroke' ? 0.32 : 0.15;
              const splashposition2 =
                this.player.getAction('swim').animationType === 'breaststroke' ? 0.1 : 0.2;
              let currentEmmit = 0;
              for (let i = 0; i < particleCount; i++) {
                if (brokenAttribute.getX(i) >= 1) {
                  this.swimmingSplash.info.velocity[i].x =
                    this.localVector2.x * (Math.random() - 0.5) * 0.2 +
                    this.playerDir.x * splashposition2 * (1 + this.currentSpeed);
                  this.swimmingSplash.info.velocity[i].y = 0.08 + Math.random() * 0.08;
                  this.swimmingSplash.info.velocity[i].z =
                    this.localVector2.z * (Math.random() - 0.5) * 0.2 +
                    this.playerDir.z * splashposition2 * (1 + this.currentSpeed);
                  positionsAttribute.setXYZ(
                    i,
                    this.particlePosition.x +
                      this.swimmingSplash.info.velocity[i].x * 0.5 +
                      this.playerDir.x * splashposition,
                    this.particlePosition.y - 0.1 * Math.random(),
                    this.particlePosition.z +
                      this.swimmingSplash.info.velocity[i].z * 0.5 +
                      this.playerDir.z * splashposition
                  );
                  ...
                  else brokenAttribute.setX(i, 0.25 + Math.random() * 0.2);
                  textureRotationAttribute.setX(i, Math.random() * 2);
                  currentEmmit++;
                }
                if (currentEmmit >= 2) {
          ...
          if (this.contactWater) {
            if (
              this.player.avatarCharacterSfx.currentStep !== this.lastStep &&
              ((this.currentSpeed <= 0.5 &&
                this.waterSurfacePos.y <
                  this.player.position.y - this.player.avatar.height * 0.7) ||
                (this.currentSpeed > 0.5 &&
                  this.waterSurfacePos.y <
                    this.player.position.y - this.player.avatar.height * 0.6))
            ) {
              let currentEmmit = 0;
              for (let i = 0; i < particleCount; i++) {
                if (brokenAttribute.getX(i) >= 1) {
                  this.swimmingSplash.info.velocity[i].x =
                    (Math.random() - 0.5) * 0.1 * (1 + this.currentSpeed);
                  this.swimmingSplash.info.velocity[i].y = 0.2 * (1 + this.currentSpeed);
                  this.swimmingSplash.info.velocity[i].z =
                    (Math.random() - 0.5) * 0.1 * (1 + this.currentSpeed);
                  if (this.player.avatarCharacterSfx.currentStep === 'left') {
                    positionsAttribute.setXYZ(
                      i,
                      this.particlePosition.x +
                        this.localVector2.x * 0.05 +
                        this.swimmingSplash.info.velocity[i].x +
                        this.playerDir.x * 0.35,
                      this.particlePosition.y + 0.1 * Math.random(),
                      this.particlePosition.z +
                        this.localVector2.z * 0.05 +
                        this.swimmingSplash.info.velocity[i].z +
                        this.playerDir.z * 0.35
                    );
                  } else {
                    positionsAttribute.setXYZ(
                      i,
              ...
              this.lastStep = this.player.avatarCharacterSfx.currentStep;
            }
          }
        }
        ...
        this.swimmingSplash.material.uniforms.cameraBillboardQuaternion.value.copy(
          this.camera.quaternion
        );
        this.swimmingSplash.material.uniforms.waterSurfacePos.value.copy(this.waterSurfacePos);
        this.lastStroke = this.player.avatarCharacterSfx.currentSwimmingHand;
      }
    };
    ...
    const _handleSwimmingRipple = () => {
      ...
      if (this.swimmingRipple) {
        ...
        for (let i = 0; i < particleCount; i++) {
          ...
          if (brokenAttribute.getX(i) < 1) brokenAttribute.setX(i, brokenAttribute.getX(i) + 0.01);
        }
        if (
          timestamp - this.swimmingRipple.info.lastEmmitTime >
            150 * Math.pow(1.1 - this.currentSpeed, 0.3) &&
          ...
          if (
            ...
            (!this.player.hasAction('swim') &&
              this.waterSurfacePos.y >= this.player.position.y - this.player.avatar.height * 0.7)
          ) {
            if (this.player.rotation.x !== 0) {
              playerRotationAttribute.setX(
                this.swimmingRipple.info.currentIndex,
                Math.PI + this.player.rotation.y
              );
            } else {
              playerRotationAttribute.setX(
                this.swimmingRipple.info.currentIndex,
    ...
    const _handleBubble = () => {
      ...
      if (this.bubble && this.player.avatar) {
        ...
        if (timestamp - this.bubble.info.lastEmmitTime > 100 && this.contactWater) {
          for (let i = 0; i < Math.floor(this.currentSpeed * 10 + 1) * 5; i++) {
            if (scalesAttribute.getX(i) <= 0) {
              ...
              positionsAttribute.setXYZ(
                i,
                this.playerHeadPos.x - this.waterWorldPosition.x,
                this.playerHeadPos.y - this.waterWorldPosition.y,
                this.playerHeadPos.z - this.waterWorldPosition.z
              );
        ...
        for (let i = 0; i < particleCount; i++) {
          ...
          if (this.bubble.info.startTime[i] % 2 === 0) this.bubble.info.offset[i] += 1;
          if (this.bubble.info.offset[i] >= 30) {
            this.bubble.info.offset[i] = 0;
          }
          offsetAttribute.setXY(
            i,
            5 / 6 - Math.floor(this.bubble.info.offset[i] / 6) * (1 / 6),
            Math.floor(this.bubble.info.offset[i] % 5) * 0.2
          );
          if (scalesAttribute.getX(i) > 0) scalesAttribute.setX(i, scalesAttribute.getX(i) + 0.01);
          if (
            this.bubble.info.startTime[i] > this.bubble.info.lastTime[i] ||
            positionsAttribute.getY(i) > this.particlePosition.y
          ) {
            scalesAttribute.setX(i, 0);
          }
        }
